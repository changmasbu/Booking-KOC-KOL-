...
switch ($act) {
    case 'register':
            ...
            if($user != ''){
                $result['type']     = "danger";
                $result['message']  = "<b>Lỗi!</b> Bạn đang đăng nhập";
                echo json_encode($result);
            }elseif(!$email || !$username || !$password || !$rePassword || !$fullname){
                $result['type']     = "danger";
                ...
                $result['type']     = "success";
                $result['message']  = "<b>Thành công!</b> Bạn đã đăng ký tài khoản thành công. Hệ thống sẽ chuyển trang sau vài giây.";
                echo json_encode($result);
    ...
    case 'login':
        ...
        $check_del  = $db->num_rows("SELECT * FROM `tbl_user` WHERE username = '$username' AND password = '$password' AND del = 1");

        // $check_del      = $db->num_rows("SELECT * FROM `tbl_user` WHERE del = 1");
        ...
        if($user != ''){
            ...
            $result['type']     = "danger";
            $result['message']  = "<b>Thất bại!</b> Tài khoản của bạn đã bị xóa";
            echo json_encode($result);
        }else{
            $result['type']     = "success";
    ...
    case 'change_info':
        ...
        if($user == ''){
            ...
            $db->query("UPDATE `tbl_user` SET `fullname`='$fullname' WHERE `id`='$user_id' ");
            $truyvan = "UPDATE `tbl_user` SET `fullname`='$fullname' WHERE `id`='$user_id' ";
            $result['type']     = "success";
